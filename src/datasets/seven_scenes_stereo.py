...
class SevenScenesStereoDataset(ScannetDataset):
    def __init__(self,
                 data_root,
                 data_split_file,
                 cleaned_seg_dir,
                 ...
                 min_w=1,
                 mini=None):
        super(SevenScenesStereoDataset, self).__init__(data_root,
                                             data_split_file,
                                             cleaned_seg_dir,
                                             anchor_normal_dir,
                                             semantic_mapping_dir,
        ...
        self.pseudo_gt_dir = '/mnt/Data/jiachenliu/scannet_data/seven_scenes_pseudo_gt'
    ...
    def load_pseudo_gt(self, img_path, to_tensor=True, area_thresh=1000):
        scene_id, seq_id, img_id = img_path.split('/')
        save_id = scene_id + '_' + seq_id + '_' + img_id

        mask_path = osp.join(self.pseudo_gt_dir, 
                             'masks', save_id.replace('color.png', 'npy'))
        score_path = osp.join(self.pseudo_gt_dir, 
                              'scores', save_id.replace('color.png', 'npy'))
        label_path = osp.join(self.pseudo_gt_dir, 
                              'labels', save_id.replace('color.png', 'npy'))
    ...
    def __getitem__(self, idx, visualize=False):
        ...
        scene_id = osp.join(ref_path.split('/')[0], ref_path.split('/')[1])
        ...
        ref_img_path = osp.join(self.data_root, scene_id, ref_img_id)
        ...
        src_img_path = osp.join(self.data_root, scene_id, src_img_id)
        ...
        ref_depth_path = osp.join(self.data_root, scene_id, ref_img_id.replace('color', 'depth'))
        ...
        ref_pose_path = osp.join(self.data_root, scene_id, ref_img_id.replace('color.png', 'pose.txt'))
        ref_pose = self.load_pose(ref_pose_path)

        if not self.is_train:
            target.add_field('ref_pose', torch.from_numpy(ref_pose))

        src_pose_path = osp.join(self.data_root, scene_id, src_img_id.replace('color.png', 'pose.txt'))
