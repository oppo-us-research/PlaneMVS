...
# copy from https://github.com/yanx27/Pointnet_Pointnet2_pytorch/
# Under license: MIT
...
if __name__ == '__main__':
    model = PointNetModel(1).cuda()
    xyz = torch.rand(12, 3, 784).cuda()

    result, trans_feat = model(xyz)
    pseudo_label = torch.ones(result.size(0), result.size(1), 1).cuda()

    for i in range(1000000):
        loss = F.binary_cross_entropy_with_logits(result, pseudo_label)
