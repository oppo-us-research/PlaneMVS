...
// based on the issue #72807 for 
// THCCeilDiv, THCudaCheck, THCudaFree, and THCudaMalloc;
// (see https://github.com/pytorch/pytorch/issues/72807#issuecomment-1699371013) 
// ------------------------------
// Why: THC was removed by PyTorch (>= 1.11);
// How: with that removed the following changes were also made to be compiled:
//   1) Add these two header files, and remove #include <THC/THC.h>
//      #include <ATen/ceil_div.h>
//      #include <ATen/cuda/ThrustAllocator.h>

//   2) Replace the following:
//     a) THCudaCheck --> AT_CUDA_CHECK: to replace THCudaCheck with AT_CUDA_CHECK
//     b) THCCeilDiv  --> at::ceil_div: to replace THCCeilDiv with at::ceil_div
//     c) THCudaFree: to replace THCudaFree(state, mask_dev); 
//         with c10::cuda::CUDACachingAllocator::raw_delete(mask_dev);
//     d) THCudaMalloc: to replace THCudaMalloc(state, boxes_num * col_blocks * sizeof(unsigned long long)); 
//         with c10::cuda::CUDACachingAllocator::raw_alloc(boxes_num * col_blocks * sizeof(unsigned long long));
